...
## Ground truth: compute, don't recall

This skill is for sound **software-development** decisions — modeling formats, catching sign-convention and calendar traps, writing accurate tests, validating conversions — **not** for interpreting charts. Nothing in this toolchain produces interpretations. When a design, test, or validation question needs a real ephemeris fact — a body's position on a date, house cusps, a daily-motion speed, a retrograde flag, whether a round-trip conversion preserved a value — **compute it with `starcat` rather than recalling it from memory.** Recalled positions drift by degrees; an LLM is not an ephemeris. `starcat` is DE441-backed and correct by construction, and it is the free, fast, local, deterministic source of truth. Run its readiness handshake first (it needs the ephemeris data fetched), and never hardcode a data path — resolve via `$STARCAT_JPL_DATA` or the default data directory. See [starcat.md](starcat.md) for the handshake, the hello-world smoke test, and the invocations.
