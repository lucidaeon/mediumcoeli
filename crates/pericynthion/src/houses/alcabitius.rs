#![allow(clippy::similar_names)]

//! Alcabitius house system.
//!
//! Trisects the equatorial semi-arcs of the ASC point (DSA above the
//! horizon, NSA below) and projects the resulting RA divisions back to
//! the ecliptic.
//!
//! Oracle: `skills/astrologer/fixtures/ref_anna_freud_alcabitius.md`.

use super::HouseCusps;
use crate::coords::acds::ac_rad;
use crate::coords::mcic::{ic_rad, mc_rad};
use std::f64::consts::{FRAC_PI_2, PI, TAU};

/// Alcabitius house cusps from RAMC, obliquity, and observer latitude.
///
/// Returns `None` for `|lat| >= 90°` and when the ASC point is
/// circumpolar (|tan(φ) · `tan(δ_asc)`| >= 1).
#[must_use]
pub fn alcabitius_rad(ramc_rad: f64, obliquity_rad: f64, lat_rad: f64) -> Option<HouseCusps> {
    if lat_rad.abs() >= FRAC_PI_2 {
        return None;
    }
    let eps_cos = obliquity_rad.cos();
    let eps_sin = obliquity_rad.sin();
    let lat_tan = lat_rad.tan();

    let mc = mc_rad(ramc_rad, obliquity_rad);
    let ac = ac_rad(ramc_rad, obliquity_rad, lat_rad)?;
    let ic = ic_rad(mc);
    let ds = (ac + PI).rem_euclid(TAU);

    // Ascensional difference of the ASC point.
    let sin_delta_asc = (eps_sin * ac.sin()).clamp(-1.0, 1.0);
    let delta_asc = sin_delta_asc.asin();
    let ad_arg = lat_tan * delta_asc.tan();
    if ad_arg.abs() >= 1.0 {
        return None; // ASC is circumpolar — Alcabitius undefined here.
    }
    let ad_asc = ad_arg.asin();
    let dsa = FRAC_PI_2 + ad_asc;
    let nsa = FRAC_PI_2 - ad_asc;

    // Project an RA offset back onto the ecliptic.
    let project = |h_rad: f64| -> f64 {
        let ra = (ramc_rad + h_rad).rem_euclid(TAU);
        f64::atan2(ra.sin(), ra.cos() * eps_cos).rem_euclid(TAU)
    };

    let h11 = project(dsa / 3.0);
    let h12 = project(2.0 * dsa / 3.0);
    let h2 = project(dsa + nsa / 3.0);
    let h3 = project(dsa + 2.0 * nsa / 3.0);
    let h5 = (h11 + PI).rem_euclid(TAU);
    let h6 = (h12 + PI).rem_euclid(TAU);
    let h8 = (h2 + PI).rem_euclid(TAU);
    let h9 = (h3 + PI).rem_euclid(TAU);

    Some(HouseCusps([
        ac, h2, h3, ic, h5, h6, ds, h8, h9, mc, h11, h12,
    ]))
}

#[cfg(test)]
mod tests {
    use super::*;
    use approx::assert_abs_diff_eq;

    const J2000_EPS_RAD: f64 = 0.409_092_62;

    #[test]
    fn polar_returns_none() {
        assert!(alcabitius_rad(0.0, J2000_EPS_RAD, FRAC_PI_2).is_none());
        assert!(alcabitius_rad(0.0, J2000_EPS_RAD, -FRAC_PI_2).is_none());
    }

    #[test]
    fn h1_h4_h7_h10_match_standard_angles() {
        let ramc = 200.0_f64.to_radians();
        let eps = J2000_EPS_RAD;
        let lat = 40.0_f64.to_radians();
        let hc = alcabitius_rad(ramc, eps, lat).unwrap();
        let ac = ac_rad(ramc, eps, lat).unwrap();
        let mc = mc_rad(ramc, eps);
        assert_abs_diff_eq!(hc.cusp(1), ac, epsilon = 1e-10);
        assert_abs_diff_eq!(hc.cusp(10), mc, epsilon = 1e-10);
        assert_abs_diff_eq!(hc.cusp(4), ic_rad(mc), epsilon = 1e-10);
        assert_abs_diff_eq!(hc.cusp(7), (ac + PI).rem_euclid(TAU), epsilon = 1e-10);
    }

    #[test]
    fn opposite_cusps_are_180_apart() {
        let ramc = 200.0_f64.to_radians();
        let eps = J2000_EPS_RAD;
        let lat = 40.0_f64.to_radians();
        let hc = alcabitius_rad(ramc, eps, lat).unwrap();
        for (a, b) in [(1_u8, 7_u8), (2, 8), (3, 9), (4, 10), (5, 11), (6, 12)] {
            let diff = (hc.cusp(b) - hc.cusp(a)).rem_euclid(TAU);
            assert_abs_diff_eq!(diff.to_degrees(), 180.0, epsilon = 1e-9);
        }
    }

    #[test]
    fn cusps_partition_the_circle() {
        let ramc = 200.0_f64.to_radians();
        let eps = J2000_EPS_RAD;
        let lat = 40.0_f64.to_radians();
        let hc = alcabitius_rad(ramc, eps, lat).unwrap();
        let mut total = 0.0_f64;
        for i in 0..12 {
            let span = (hc.0[(i + 1) % 12] - hc.0[i]).rem_euclid(TAU).to_degrees();
            assert!(span > 0.0 && span < 180.0, "H{} span {:.3}°", i + 1, span);
            total += span;
        }
        assert_abs_diff_eq!(total, 360.0, epsilon = 1e-6);
    }
}
